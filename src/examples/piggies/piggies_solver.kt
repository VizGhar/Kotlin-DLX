    ...
    for (tile in tiles) {
        for (offsetY in 0..map.size - tile.shape.size) {
            for (offsetX in 0..map[0].length - tile.shape[0].length) {
                ...
                val cellCoverage = tile.shape.mapIndexed { y, s -> s.mapIndexedNotNull { x, c -> if (c == '.') null else Requirement.CellCovered(x + offsetX, y + offsetY) } }.flatten()
...
fun main() {
    createSolver((0..3).map { readln() }).solve()
}