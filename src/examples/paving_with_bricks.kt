...
private object Paving {
    ...
    fun createSolver(height: Int, width: Int): PavingWithBricksSolver {
        ...
        for (y in 0..<height) {
            for (x in 0..<width) requirements += Requirement.CellCovered(x, y)
        }

        for (y in 0..<height) {
            for (x in 0..<width) {
                if (x + 1 < width) actions[Action(x, y, x + 1, y)] =
                    listOf(Requirement.CellCovered(x, y), Requirement.CellCovered(x + 1, y))
                if (y + 1 < height) actions[Action(x, y, x, y + 1)] =
                    listOf(Requirement.CellCovered(x, y), Requirement.CellCovered(x, y + 1))
            }
        }
...
fun solvePavingWithBricks() {
    Paving.createSolver(readln().toInt(), readln().toInt()).solve()
}