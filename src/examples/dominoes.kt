...
private object Dominoes {
    ...
    fun createSolver(n: Int, height: Int, width: Int, board: List<List<Int>>): DominoesSolver {
        ...
        for (y in 0..height - 1) {
            for (x in 0..width - 1) requirements += Requirement.CellTaken(x, y)
        }
        ...
        for (domino in dominoes) {
            ...
            for (y in 0..height - 1) {
                for (x in 0..width - 2) {
                    if (board[y][x] == domino.first && board[y][x + 1] == domino.second) actions[Action(domino, x, y, x + 1, y, '=')] = listOf(
                        Requirement.CellTaken(x, y),
                        Requirement.CellTaken(x + 1, y),
                        Requirement.DominoPlaced(domino)
                    )
                }
            }
            for (y in 0..height - 1) {
            ...
            for (y in 0..height - 2) {
                for (x in 0..width - 1) {
                    if (board[y][x] == domino.first && board[y + 1][x] == domino.second) actions[Action(domino, x, y, x, y + 1, '|')] = listOf(
                        Requirement.CellTaken(x, y),
                        Requirement.CellTaken(x, y + 1),
                        Requirement.DominoPlaced(domino)
                    )
                }
            }
...
fun solveDominoes() {
    ...
    scanner.nextLine()
    Dominoes.createSolver(n, h, w, List(h) { scanner.nextLine().map { it.digitToInt() } }).solve()
}
