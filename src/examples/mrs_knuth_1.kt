...
private object MrsKnuth1 {
    ...
    fun createSolver(teacherCalendar: Map<String, List<Int>>, studentInfo: List<StudentInfo>): MrsKnuth1Solver {
...
fun solveMrsKnuth1() {
    val teacherSchedule = MrsKnuth1.parseCalendar(readln().split(" "))
    val studentInfos = (0..<readln().toInt()).map {
        val d = readln().split(" ")
        val (name, instrument) = d.take(2)
        MrsKnuth1.StudentInfo(name, instrument, MrsKnuth1.parseCalendar(d.drop(2)))
    }

    MrsKnuth1.createSolver(teacherSchedule, studentInfos).solve()
}